function getrecipe(q) {
    $.ajax({
        url: "https://api.spoonacular.com/recipes/search?apiKey=a568c4a88785422fbf4cf46b976c40e8&number=3&query=" + q,
        success: function (res) {
            for (var i = 0; i < 3; i++) {
                document.getElementById("output").innerHTML += "<h1>" + res.results[i].title + "</h1><br><a href='" + res.results[i].sourceUrl + "'><img src='" + res.baseUri + res.results[i].image + "' width='400' /></a><br>Ready in " + res.results[i].readyInMinutes + " minutes"
                console.log(q);
            }
        }
    });
